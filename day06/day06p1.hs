-- --- Day 6: Wait For It ---
-- The ferry quickly brings you across Island Island. After asking around, you discover that there is indeed normally a large pile of sand somewhere near here,
...
-- You manage to sign up as a competitor in the boat races just in time. The organizer explains that it's not really a traditional race -
...
-- Your toy boat has a starting speed of zero millimeters per millisecond. For each whole millisecond you spend at the beginning of the race holding down the button,
...
mainFunc input =
  let inputLines = lines input
      times = splitToInts $ head inputLines
      distances = splitToInts $ inputLines !! 1
      races = [Race t d | (t, d) <- zip times distances]
      winners = [winningDistances r | r <- races]
      wincounts = [length x | x <- winners]
      answer = product wincounts
   in "Times:"
        ++ show times
        ++ "\nDistances:"
        ++ show distances
...
winningDistances :: Race -> [Int]
winningDistances race =
  let time = raceTime race
      record = recordDistance race
      holdTimes = [1 .. time - 1]
      distances = [(time - x) * x | x <- holdTimes]
      winners = filter (> record) distances
   in winners

splitToInts :: String -> [Int]
splitToInts s =
  let parts = drop 1 $ splitOn' ' ' s
      filtered = catMaybes [if x /= "" then Just x else Nothing | x <- parts]
      ints = [read x :: Int | x <- filtered]
   in ints
