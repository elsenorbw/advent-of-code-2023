...
mainFunc input =
  let zone = loadMirrorZone input
      startingOptions = generateStartingPositionsForZone zone
      energyCounts = [getEnergizedCountForStartPos zone x | x <- startingOptions]
      result = findMax energyCounts
   in "\n"
        ++ show zone
        ++ "\nResult : "
        ++ show result
...
generateStartingPositionsForZone :: MirrorZone -> [(Location, Direction)]
generateStartingPositionsForZone zone =
  let leftSide = [(Location 0 y, DirRight) | y <- [0 .. maxY zone]]
      rightSide = [(Location (maxX zone) y, DirLeft) | y <- [0 .. maxY zone]]
      topSide = [(Location x 0, DirDown) | x <- [0 .. maxX zone]]
      bottomSide = [(Location x (maxY zone), DirUp) | x <- [0 .. maxX zone]]
   in concat [leftSide, rightSide, topSide, bottomSide]

getEnergizedCountForStartPos :: MirrorZone -> (Location, Direction) -> Int
getEnergizedCountForStartPos zone startPos =
  let starters = [startPos]
