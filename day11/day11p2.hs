...
mainFunc input =
  let universe = loadUniverse input
      expandedUniverse = expandUniverse universe 1_000_000
...
expandUniverse :: Universe -> Int -> Universe
expandUniverse universe expansionSize =
  let (minX, maxX, minY, maxY) = getUniverseSize universe
      ...
      universeA = foldl (expandUniverseY expansionSize) universe expandYs
      universeB = foldl (expandUniverseX expansionSize) universeA expandXs
   in universeB
  where
    noGalaxies (idx, galaxyCount) = galaxyCount == 0
    revsort = reverse . sort

expandUniverseX :: Int -> Universe -> Int -> Universe
expandUniverseX expansionSize universe targetX =
  let newGalaxyLocations = [if theX g < targetX then g else Galaxy (expansionSize - 1 + theX g) (theY g) | g <- galaxies universe]
   in Universe newGalaxyLocations

expandUniverseY :: Int -> Universe -> Int -> Universe
expandUniverseY expansionSize universe targetY =
  let newGalaxyLocations = [if theY g < targetY then g else Galaxy (theX g) (expansionSize - 1 + theY g) | g <- galaxies universe]
   in Universe newGalaxyLocations
