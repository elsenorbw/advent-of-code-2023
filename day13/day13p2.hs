...
-- Upon closer inspection, you disc>over that every mirror has exactly one smudge: exactly one . or # should be the opposite type.
...
-- 6 ..##..##. 6
-- 7 #.#.##.#. 7
-- With the smudge in the top-left corner repaired, a new horizontal line of reflection between rows 3 and 4 now exists. Row 7 has no corresponding 
-- reflected row and can be ignored, but every other row matches exactly: row 1 matches row 6, row 2 matches row 5, and row 3 matches row 4.
...
-- Summarize your notes as before, but instead use the new different reflection lines. In this example, the first pattern's new horizontal line has 
-- 3 rows above it and the second pattern's new horizontal line has 1 row above it, summarizing to the value 400.

-- In each pattern, fix the smudge and find the different line of reflection. 
-- What number do you get after summarizing the new reflection line in each pattern in your notes?


-- Lazy option: iterate all the different options for each cave and run them.. although this is very 
-- Smarter option: write a version of the compare that counts errors when it splits, identify the answer with exactly one error

-- Going to try the smarter option, doesn't sound that hard (hah)
...
mainFunc input =
  let caves = loadCaveMaps input
      horizontalMatches = [findSmudgedMirrors 1 (rows x) | x <- caves]
      verticalMatches = [findSmudgedMirrors 1 (columns x) | x <- caves]
...
findSmudgedMirrors :: Int -> [String] -> Int 
findSmudgedMirrors chopPoint xs
  | chopPoint >= length xs = 0
  | otherwise =
      let a = reverse $ take chopPoint xs
          b = drop chopPoint xs
          matcher = zip a b
          diffcounts = [sum [if a1 == b1 then 0 else 1 | (a1, b1) <- zip a b] | (a, b) <- matcher]
       in if sum diffcounts == 1 then chopPoint else findSmudgedMirrors (chopPoint + 1) xs
