...
-- let you borrow my boat and you can go visit the gardener. I got all these scratchcards as a gift, but I can't figure
-- out what I've won."
...
-- ok - so this is obviously cacheable from the bottom up. If we know how many cards each one is worth then in the example above, if you come from the bottom you get :
-- Card 6 = 1 (Itself)
-- Card 5 = 1 (Itself)
-- Card 4 = 1 (Itself) + (1 Match) => Card 5 == 1 = 1 ... Card 4 = 2
-- Card 3 = 1 (Itself) + (2 Matches) => Card 4 + Card 5 == 2+1 = 3 ... Card 3 = 4
-- Card 2 = 1 (Itself) + (2 Matches) => Card 3 + Card 4 == 4+2 = 6 ... Card 2 = 7
-- Card 1 = 1 (Itself) + (4 Matches) => Card 2 + Card 3 + Card 4 + Card 5 == 7+4+2+1 = 14 ... Card 1 = 15

-- sum is 1+1+2+4+7+15=30

-- simple, fast, no messing about, plus we can use what we have already done to mostly complete it

main = interact mainFunc

mainFunc :: String -> String
mainFunc input =
  let theCards = loadCards $ lines input
      theScores = [scoreCard c | c <- theCards]
      counts = generateCardCounts theScores []
      total = sum counts
   in show "Cards:" ++ show theCards ++ "\n" ++ "Scores:" ++ show theScores ++ "\n" ++ "Counts: " ++ show counts ++ "\n" ++ "Total: " ++ show total ++ "\n"
...
generateCardCounts :: [Int] -> [Int] -> [Int]
generateCardCounts [] x = x
generateCardCounts cards scoresSoFar =
  let lastCard = last cards
      remainingCards = take (length cards - 1) cards
      thisScore = sum $ 1 : take lastCard scoresSoFar
   in generateCardCounts remainingCards (thisScore : scoresSoFar)

scoreCard :: Card -> Int
scoreCard c =
  let matches = [if x `elem` winners c then 1 else 0 | x <- chosen c]
      match_count = sum matches
   in match_count

loadCards :: [String] -> [Card]
loadCards lines = [loadOneCard s | s <- lines]

loadOneCard :: String -> Card
loadOneCard s =
  let withoutCardNo = splitOnChar ':' s !! 1
      (allWinners : allChoices : _) = splitOnChar '|' withoutCardNo
      winnerInts = spaceListToInts allWinners
      choiceInts = spaceListToInts allChoices
   in Card winnerInts choiceInts

-- Utility
spaceListToInts :: String -> [Int]
spaceListToInts s =
  let chopped = splitOnChar ' ' s
      noBlanks = filter notEmpty chopped
      inted = [read x :: Int | x <- noBlanks]
   in inted

notEmpty :: String -> Bool
notEmpty "" = False
notEmpty (x : xs) = if ' ' == x then notEmpty xs else True
