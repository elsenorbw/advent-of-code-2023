...
handRanker = rankerHighCard . rankerPairs . rankerTrips . rankerFull . rankerFour . rankerFive

rankerFive :: Hand -> Hand
rankerFive hand =
  if rank hand == rankUnknown
    then
      let cardStr = cards hand
       in if allSame cardStr then Hand cardStr (bid hand) rankFiveOfAKind else hand
    else hand
...
rankerFour hand =
  if rank hand == rankUnknown
    then
      let cardStr = cards hand
          sortedCards = sort cardStr
          a = take 4 sortedCards
          b = drop 1 sortedCards
       in if allSame a || allSame b then Hand cardStr (bid hand) rankFourOfAKind else hand
    else hand
...
rankerFull hand =
  if rank hand == rankUnknown
    then
      let cardStr = cards hand
          sortedCards = sort cardStr
          a1 = take 3 sortedCards
          a2 = drop 3 sortedCards
          b1 = take 2 sortedCards
          b2 = drop 2 sortedCards
       in if (allSame a1 && allSame a2) || (allSame b1 && allSame b2) then Hand cardStr (bid hand) rankFullHouse else hand
    else hand
...
rankerTrips hand =
  if rank hand == rankUnknown
    then
      let cardStr = cards hand
          frequencies = cardFrequencies cardStr
          trips = filter (== 3) frequencies
       in if 1 == length trips then Hand cardStr (bid hand) rankTrips else hand
    else hand

rankerPairs :: Hand -> Hand
rankerPairs hand =
  if rank hand == rankUnknown
    then
      let cardStr = cards hand
          frequencies = cardFrequencies cardStr
          pairs = filter (== 2) frequencies
       in if 2 == length pairs then Hand cardStr (bid hand) rankTwoPair else if 1 == length pairs then Hand cardStr (bid hand) rankOnePair else hand
    else hand
...
addFrequency (x : xs) freqs =
  let cardIndex = cardToIndex x
      updatedList = incrementListItem freqs cardIndex
