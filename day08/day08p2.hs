...
mainFunc input =
  let instructions = head $ lines input
      ...
      steps = [walkToMultipleFinishPoints x locationMap (cycle instructions) 0 3 | x <- startLocations]
      ...
      result = findLoopIntersect loopIntervals 0
   in "Instructions: "
        ++ show instructions
        ++ "\nlocationMap:\n"
        ++ show locationMap
...
findLoopIntersect :: [Int] -> Int -> Int
findLoopIntersect (increment : xs) startValue =
  let nextValue = startValue + increment
      mods = [mod nextValue x | x <- xs]
   in if sum mods == 0 then nextValue else findLoopIntersect (increment : xs) nextValue
